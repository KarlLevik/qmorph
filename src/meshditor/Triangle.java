...
/**
 * A class holding information for triangles, and with methods for the handling of
 * issues regarding triangles.
*/

public class Triangle extends Element {
    public Triangle(Edge edge1,Edge edge2,Edge edge3,double len1,double len2,double len3,double ang1,double ang2,double ang3, boolean lengthsOpt, boolean anglesOpt) {
	edgeList = new Edge[3];
    ...
    // Create a simple triangle for testing purposes only (constrainedLaplacianSmooth()
    // and optBasedSmooth(..))
    // The triangle created is a copy of this, and each of the edges are also copies.
    ...
    // Return true if the quad becomes inverted when node n is relocated to pos. (x,y).
    // Else return false.
	...
	if ((edgeList[0]== e1 && edgeList[1]== e2) ||
	    (edgeList[1]== e1 && edgeList[0]== e2))
	    return edgeList[2];
	else if ((edgeList[1]== e1 && edgeList[2]== e2) || 
		 (edgeList[2]== e1 && edgeList[1]== e2))
	    return edgeList[0]; 
	else if ((edgeList[0]== e1 && edgeList[2]== e2) || 
		 (edgeList[2]== e1 && edgeList[0]== e2))
	    return edgeList[1]; 
	else {
    ...
    // Release the element pointer of each Edge in edgeList that pointed to this Element.
    ...
    // Get the pos angles between e1 and each of the other two edges. Transform these
    // angles into true angles. The largest angle indicates that this edge is ccw to e1.
	...
	/*
	Node n= oppositeOfEdge(edgeList[0]);
	if (n.onLine(edgeList[0]))
	    return false;
	else
	    return true;
	*/
     ...
     * Equilateral triangles will return the maximum value of 1.
     */
	...
	Node a= edgeList[2].commonNode(edgeList[0]), 
	    b= edgeList[0].commonNode(edgeList[1]), 
