...
class GCanvas extends Canvas {
	...
	double rounded_ymaxXscale= signOf(ymax)*
	    (Math.abs(ymaxXscale)+gridIncr-
	    Math.IEEEremainder(Math.abs(ymaxXscale), (double)gridIncr)); 
	double rounded_xminXscale= signOf(xmin)*
	    (Math.abs(xminXscale)+gridIncr-
	    Math.IEEEremainder(Math.abs(xminXscale), (double)gridIncr)); 
	...
	if (gui.grid) {
	    ...
	    for (int i= 0; i< (int)getWidth(); i+=gridIncr) 
		g.drawLine(i,0,  i,getHeight());
	    
	    for (int i= 0; i< (int)getHeight(); i+=gridIncr) 
	...
	if (gui.axis) {
	    ...
	    for (int i= yaxis_xval; i< (int)getWidth(); i+=gridIncr*5) {
		g.drawLine(i, xaxis_yval+halfGridIncr, i, xaxis_yval);
		g.drawString(Double.toString(((double)(i-yaxis_xval))/scale), 
			     i, xaxis_yval+gridIncr);
	    }
		...
		g.drawString(Double.toString(((double)(xaxis_yval-i))/scale),
			     yaxis_xval-halfGridIncr, i);
	    }
	...
	if (edgeList!= null) {
		...
		if (e.color== java.awt.Color.red)
		    g.setColor(e.color);
		else if (e.leftNode.edgeList.indexOf(e)== -1 && 
		    e.rightNode.edgeList.indexOf(e)== -1)
		...
		g.drawLine((int)(e.leftNode.x*scale+yaxis_xval), 
			   (int)(-e.leftNode.y*scale+xaxis_yval), 
	...
	if (nodeList!= null) {
		...
		g.setColor(n.color);
		g.fillOval((int)(n.x*scale+yaxis_xval-3), 
			   (int)(-n.y*scale+xaxis_yval-3),
			   5, 
			   5);
	    }
	}
