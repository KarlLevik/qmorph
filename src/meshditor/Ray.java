...
/**
 This class holds information for rays, and has methods for dealing with ray-related 
 issues. The purpose of this class is solely to determine the intersection point between
 a ray (origin and direction) and a vector (origin and x,y giving the direction, the 
 length of the ray is considered to be infinite).
*/
...
public class Ray {
	...
	Msg.debug("Ray(..): relEdge.angleAt(origin)+ angle== "
		  +Math.toDegrees(ang)+" degrees");
	...
	if (d0crossd1== 0) {
		...
		if (d0.origin.equals(d1.origin) || (d0.origin.x== d1.origin.x +d1.x && 
						    d0.origin.y== d1.origin.y +d1.y))
		    return d0.origin;
	    return null;   
	}
	else {
