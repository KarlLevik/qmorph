...
/**
  This class holds information for edges, and has methods for handling issues involving edges.
*/
...
public class Edge extends Constants {
	...
	Element tri= (Element)getTriangleElement(), quad= getQuadElement();
	double ang;
	...
	Msg.debug("sumAngle(..) between "+descr()+" and "+frontNeighbor.descr()+": "+
		  Math.toDegrees(ang));
    ...
    // Edge this as a side Edge at that Node. If it is not set, then it must get a null
    // value instead.
    
    // this: the edge to be classified (gets a state value, and is added to a statelist)
	...
	if (selState!= 2) {
		...
		if (selected.length() > selected.leftFrontNeighbor.length() &&
		    selected.leftFrontNeighbor.selectable)
	...
	if (Msg.debugMode) {
	    ...
	    for (int i= 0; i< stateList[2].size(); i++) {
		Edge edge = (Edge)stateList[2].get(i);
		System.out.println(""+edge.descr()+", ("+edge.getState()+")");
	    }
	    System.out.println("frontsInState 0-1 and 1-0:");
	    for (int i= 0; i< stateList[1].size(); i++) {
		Edge edge = (Edge)stateList[1].get(i);
		System.out.println(""+edge.descr()+", ("+edge.getState()+")");
	    }
	    System.out.println("frontsInState 0-0:");
	    for (int i= 0; i< stateList[0].size(); i++) {
		Edge edge = (Edge)stateList[0].get(i);
		System.out.println(""+edge.descr()+", ("+edge.getState()+")");
	    }
	}
    }
	...
	if ((leftNode.x < e.leftNode.x) || 
	    (leftNode.x==e.leftNode.x && leftNode.y < e.leftNode.y)) 
	    return true;
	else 
	    return false;
    }
	...
	/*
	  ...
	  (element1 instanceof Triangle && element2 instanceof Triangle))
	  return false;
	  else
	  return true;
	*/
    ...
    // Provided that otherThis or the otherNode of otherE is not found in any edge in 
    // the edgeList of the otherNode of this edge (otherThis), then the edge is added 
    // to this' edgeList.

    // Assumes that the quad area defined by the three distinct nodes of the two edges, 
    // and the node in the top of the triangle adjacent the otherNodes (of the common 
    ...
    // Returns angle relative to the x-axis (which is directed from the origin (0,0) to
	...
	if (x== 0 && y> 0)
	    ...
	    if (y> 0)
		return -Math.acos((aLen*aLen + x*x -y*y)/(2*aLen*Math.abs(x)));
	    else
		return Math.acos((aLen*aLen + x*x -y*y)/(2*aLen*Math.abs(x)));
	    */
	}
    ...
    // Returns the angle from this Edge to eEdge by summing the angles of the Elements
    // adjacent Node n.
    ...
    // Remove this Edge from the nodes' edgeLists. Careful, there's no safety checks! 
	...
	if (hasElement((Element)triangle))
	    return;
	if (element1==null) 
	    element1= triangle;
  ...
  */
    ...
    /** Swap diagonal between the edge's two triangles and update locally 
     * (To be used with getSwappedEdge()) */
	...
	if (leftFrontNeighbor== null || !leftFrontNeighbor.frontEdge || 
	    rightFrontNeighbor== null || !rightFrontNeighbor.frontEdge)
	    return true;
	else
	    return false;
    }
     ...
     * the boundary, and the orbit contains a hole, the orbit simply skips the hole and
	...
	if (list.size()== 1)
	    ...
	    t= getTriangleElement();
	    for (int i= 0; i< list.size(); i++) {
		rightEdge=(Edge)list.get(i);
		curAng= sumAngle(t, rightNode, rightEdge);

		Msg.debug("findRightFrontNeighbor(): Angle between edge this: "+descr()+
			  " and edge "+rightEdge.descr()+ ": "+curAng);
		if (curAng< candAng) {
		    candAng= curAng;
		    candidate= rightEdge;
		}
    ...
    /** Halve this Edge by introducing a new Node at the midpoint, and
     * create two Edges from this midpoint to the each of the two opposite Nodes of 
     * Edge this: one in element1 and one in element2.
     * Also create two new Edges from Node mid to the two Nodes of Edge this.
     * Create four new Triangles.
     * Update everything (also remove this Edge from edgeList and disconnect the nodes).
     ...
     * @return the "lower" (the one incident with the baseEdge) of the two edges created
     * from splitting this edge.
     */
     ...
     * part of a  given element and which is adjacent a given node. 
     * Note that the method stops if the boundary is encountered.
     * @param n the node
     * @param startElem 
     * @return the first edge of a quad found when parsing around node n, starting at 
     * edge e in element startElem and moving in the direction from e to e's neighbor 
     * edge at n in startElem. If startElem happens to be a quad, the method won't 
     * consider that particular quad. If unsuccessful, the method returns null.
     */
    ...
    // Returns a neighboring element that is a quad. When applied to inner front edges,
    ...
    // Returns a neighboring element that is a triangle. The method should work as long
    ...
    // Return a neighbor edge at node n, that is front edge according to the definition,
