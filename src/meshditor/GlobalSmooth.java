...
// ==== ---- ==== ---- ==== ---- ==== ---- ==== ---- ==== ---- ==== ---- 
/**
 * This class is an implementation of the algorithm
 * described in the paper "An approach to Combined Laplacian and
 * Optimization-Based Smoothing for Triangular, Quadrilateral and
 * Quad-Dominant Meshes" (1998) by Cannan, Tristano, and Staten.
 * 
 * The meshes produced by Q-Morph are indeed highly quad-dominant,
 * with at most one single triangle, so Q-Morph should work well with 
 * this algorithm.
 * 
...
public class GlobalSmooth extends GeomBasics {
	...
	for (int count= 0; count< 20; count++) {
		...
		if ((oElem.distortionMetric< 0 && sElem.distortionMetric>=0)|| 
		    (oElem.distortionMetric< 0 && 
		     sElem.distortionMetric> oElem.distortionMetric) ||
		    (oElem.distortionMetric< MYMIN && 
		     sElem.distortionMetric>= MYMIN))
		    Nup++;
		// # elements whose metric worsens significantly
    ...
    /** @return true if the new constrained-smoothed position is acceptable according to 
     * the criteria given in section 4.2 of the article. */
    private boolean acceptable(int N, int Nminus, int Nplus, int Nup, int Ndown, 
			       int Ninverted, double deltaMy, double theta) {
	...
	if (Nminus== N || Ninverted> 0 || Ndown> Nup || 
	    deltaMy< MYMIN || theta>THETAMAX)
	    return false;
     ...
     * altered.  
     * @return a node with a position that is the optimaization-based smoothed position
     * of node n. */
	...
	do {  // Iterate until the min. dist. metric is acceptable
		...
		if ((Math.abs(oElem.gX)> 0.00001 || Math.abs(oElem.gY)> 0.00001) &&
		    oElem.distortionMetric < minDM) {
	    ...
	    boolean flag= false;
	    double gamma= java.lang.Double.MAX_VALUE, gammaI= 0, 
		gdotgi, gdotg= gX*gX +gY*gY; 
