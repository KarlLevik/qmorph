...
/**
 * A class holding information for quadrilaterals, and with methods for the handling of
 * issues regarding quads.
*/
...
public class Quad extends Element {
    ...
    /** Create ordinary quad.
     * Use Edge e's two elements (Triangles) as basis.
	...
	if (edgeList[base].leftNode== e.leftNode) {
	    ...
	    edgeList[left]= new Edge(edgeList[base].leftNode, 
				     edgeList[top].otherNode(e.rightNode));
	}
	else {
	    edgeList[right]= new Edge(edgeList[base].rightNode, 
				      edgeList[top].otherNode(e.rightNode));
     ...
     * Create fake quad with only three nodes */
    ...
    /** @return true if the quad becomes inverted when node n1 is relocated to pos. n2.
     * Else return false. */
	...
	Node a= edgeList[base].leftNode, 
	    b= edgeList[base].rightNode,
	    c= edgeList[right].otherNode(b),
    ...
    /** Test whether any neighboring elements becomes inverted if the quad is collapsed 
     * in a particular manner.
     * @param q the quad to be collapsed
     * @param n a node holding the position for where the joined nodes are to be located 
     ...
     * @return true if any elements adjacent to quad q becomes inverted when collapsing 
     * quad q, joining its two opposite nodes n1 and n2 to the position held by 
     * node n. Node n must be located somewhere inside quad q. */
    public boolean anyInvertedElementsWhenCollapsed(Node n, Node n1, Node n2, 
     ...
     * the edgeList of the otherNode (otherE2) of edge e2:
     * Provided that otherE1 or the otherNode of otherE2 is not found in any edge in 
     * the edgeList of the otherNode of edge e1 (otherE1), then the edge is added 
     * to otherE1's edgeList.
     * Assumes that the inner quad area is empty and that the edges of the quad don't
     * have the quad as an element. 
     * @param e1 first edge
	...
	for (int i= 0; i< nKm1.edgeList.size(); i++) {
		...
		Msg.debug("...replacing "+nKm1.descr()+" with "+nKp1.descr()+" on edge "
			  +eI.descr());
    ...
    /** Create a new quad by combining this quad with quad q. 
     * The two quads must initially share two incident edges:
    ...
    /** Create a new triangle by combining this quad with Triangle t. 
     * The two elements must initially share two incident edges:
     * @param e1 first common edge
     * @param e2 second common edge */
	...
	if (edgeList[base].leftNode.equals(n) || 
	    edgeList[base].rightNode.equals(n) || 
	    edgeList[top].leftNode.equals(n) ||
	...
	if (edgeList[base]== e || edgeList[left]== e || 
	    edgeList[right]== e || edgeList[top]== e)
	    return true;
	else
	    return false;
    }
	...
	if (neighbor(edgeList[base]) instanceof Quad || 
	    neighbor(edgeList[left]) instanceof Quad || 
    ...
    /** Method to verify that the quad has an area greater than 0. We simply check that 
	the nodes of the element are not colinear. */ 
    public boolean areaLargerThan0() {
	Node node3= edgeList[top].leftNode;
	Node node4= edgeList[top].rightNode; 

	if (!node3.onLine(edgeList[base])) 
	    return true;
	if (!node4.onLine(edgeList[base]))
	    return true;
	return true;
    }
    ...
    /** @return true if the quad is a bowtie, defined as a quad with two opposite edges
     * that intersect. */
    ...
    /** Update so that the edge connected to edgeList[base].leftNode is edgeList[left]
     * and that the edge connected to edgeList[base].rightNode is edgeList[right].
     * The angle between base and left is at pos 0 in the ang array. 
     * The angle between right and base is at pos 1 in the ang array.
     * The angle between left and top is at pos 2 in the ang array.
     * The angle between top and right is at pos 3 in the ang array.
     */
    ...
    /** Update the values in the ang array.
     * Works correctly only for uninverted quads. */
	...
	if (isFake) {
	    if (firstNode== edgeList[base].leftNode){	    
		if (i!= 0) ang[0]= edgeList[base].computeCCWAngle(edgeList[left]);
		if (i!= 1) ang[1]= edgeList[right].computeCCWAngle(edgeList[base]);
		if (i!= 2) ang[2]= edgeList[left].computeCCWAngle(edgeList[right]);
	    }
	    else {
		if (i!= 0) ang[0]= edgeList[left].computeCCWAngle(edgeList[base]);
		if (i!= 1) ang[1]= edgeList[base].computeCCWAngle(edgeList[right]);
		if (i!= 2) ang[2]= edgeList[right].computeCCWAngle(edgeList[left]);
	    }
	}
	else if (firstNode== edgeList[base].leftNode){
	    if (i!= 0) ang[0]= edgeList[base].computeCCWAngle(edgeList[left]);
	    if (i!= 1) ang[1]= edgeList[right].computeCCWAngle(edgeList[base]);
	    if (i!= 2) ang[2]= edgeList[left].computeCCWAngle(edgeList[top]);
	    if (i!= 3) ang[3]= edgeList[top].computeCCWAngle(edgeList[right]);
	}
	else {
	    if (i!= 0) ang[0]= edgeList[left].computeCCWAngle(edgeList[base]);
	    if (i!= 1) ang[1]= edgeList[base].computeCCWAngle(edgeList[right]);
	    if (i!= 2) ang[2]= edgeList[top].computeCCWAngle(edgeList[left]);
	    if (i!= 3) ang[3]= edgeList[right].computeCCWAngle(edgeList[top]);
	}
    }
	...
	if (isFake) {
	    if (firstNode== edgeList[base].leftNode){	    
		if (i== 0) ang[0]= edgeList[base].computeCCWAngle(edgeList[left]);
		else if (i== 1) ang[1]= edgeList[right].computeCCWAngle(edgeList[base]);
		else if (i== 2) ang[2]= edgeList[left].computeCCWAngle(edgeList[right]);
	    }
	    else {
		if (i== 0) ang[0]= edgeList[left].computeCCWAngle(edgeList[base]);
		else if (i== 1) ang[1]= edgeList[base].computeCCWAngle(edgeList[right]);
		else if (i== 2) ang[2]= edgeList[right].computeCCWAngle(edgeList[left]);
	    }
	}
	else if (firstNode== edgeList[base].leftNode){
	    if (i== 0) ang[0]= edgeList[base].computeCCWAngle(edgeList[left]);
	    else if (i== 1) ang[1]= edgeList[right].computeCCWAngle(edgeList[base]);
	    else if (i== 2) ang[2]= edgeList[left].computeCCWAngle(edgeList[top]);
	    else if (i== 3) ang[3]= edgeList[top].computeCCWAngle(edgeList[right]);
	}
	else {
	    if (i== 0) ang[0]= edgeList[left].computeCCWAngle(edgeList[base]);
	    else if (i== 1) ang[1]= edgeList[base].computeCCWAngle(edgeList[right]);
	    else if (i== 2) ang[2]= edgeList[top].computeCCWAngle(edgeList[left]);
	    else if (i== 3) ang[3]= edgeList[right].computeCCWAngle(edgeList[top]);
	}
    }
    ...
    /** Determines whether there is a concavity (angle > 180 degrees) at the specified 
     * node. 
    ...
    /** @return the centroid of this quad.... or at least a point *inside* the quad... 
     * Assumes that the quad is not inverted. */
	...
	// Parse all adjacent elements at lower right node from neigbor of right edge to,
	...
	for (i= 0; i< bLNode.edgeList.size(); i++) {
	    ...
	    if (e!= edgeList[base] && e!= edgeList[left] &&
		e!= edgeList[right] && e!= edgeList[top])
    ...
    /** Make the Element pointers in each of the Edges in this Quad
     * point to this Quad. */
    ...
    /** Release the element pointer of the edges in edgeList that pointed to this Quad.*/
    ...
    /** @return an Edge that is common to both this Quad and Quad q at Node n. 
    ...
    /** @return true if at least one of the edges connected to node n is a front edge. */
    ...
    /** @return the number of quad neighbors sharing an edge with this quad at 
     * node n. This quad is not counted. Values are 0, 1, or 2. */
    ...
    /** Update the distortion metric according to the paper "An approach to
     * Combined Laplacian and Optimization-Based Smoothing for Triangular,
     * Quadrilateral and Quad-Dominant Meshes" by by Cannan, Tristano,
     * and Staten
    ...
    //			n1           n2
    //
    // Also, I tried to sketch the case where the quad has an angle > than 180 degrees 
	...
	if (isFake) {
	    double AB= edgeList[base].len, CB= edgeList[left].len, 
		CA= edgeList[right].len;

	    Node a= edgeList[base].commonNode(edgeList[right]), 
		b= edgeList[base].commonNode(edgeList[left]), 
	...
	double alpha1= t1.distortionMetric, alpha2= t2.distortionMetric,
	    alpha3= t3.distortionMetric, alpha4= t4.distortionMetric;
    ...
    public void printMe() {
	System.out.println(descr()+", inverted(): "+inverted()+", ang[0]: "+Math.toDegrees(ang[0])+", ang[1]: "+Math.toDegrees(ang[1])+", ang[2]: "+Math.toDegrees(ang[2])+", ang[3]: "+Math.toDegrees(ang[3])+", firstNode is "+firstNode.descr());
    }
