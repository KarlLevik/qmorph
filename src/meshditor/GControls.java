...
/** The Panel class with step button, zoom menu, and axis and grid toggle buttons etc. */
class GControls extends Panel implements ItemListener {
     ...
     * changes due to user interaction. The item is identified and the required action 
     * is taken. */ 
	...
	/** Method which is automatically called when the button is pressed. 
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (method.leftmost== null)
		    method.findExtremeNodes();
		//	    canvas.repaint();
		//GeomBasics.findExtremeNodes();
		canvas.resize(method.leftmost.x, method.lowermost.y, 
			      method.rightmost.x, method.uppermost.y, 
			      gui.scale); 
	    }
	    else {
		md= new MsgDialog(gui.f, "Program message", 
				  "You must choose a method first.", 40, 1);
		md.show();
	    }	    
	}
    }
