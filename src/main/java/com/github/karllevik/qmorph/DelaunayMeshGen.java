...
/**
  This class offers methods for incrementally constructing Delaunay triangle meshes.
*/
...
public class DelaunayMeshGen extends GeomBasics {
     ...
     * @param delaunayCompliant boolean to indicate whether to create a delaunay compliant 
     * mesh or not.
     */
    ...
    /** Step through the insertions one by one. Upon completion, run incrDelaunayPost().
     */
    public void step() {
	Node n;
    ...
    /** Find the triangle (in the list) that contains the specified Node.
     * A method highly inspired by the dart based localization procedure.
     * @return the Triangle containing the Node, but if the Node is located on an Edge, 
     * this Edge is returned instead. Set inside= true if found. 
     * If the Node is located outside of the current triangulation, then set
     * inside= false and return a boundary triangle that can be seen from the node.
     */
	...
	while (true) {
	    ...
	    */
	    ...
	    /*	    else if (hp==0 && newNode.inBoundedPlane(e)) { // is newNode actually on Edge e?
		Msg.debug("Leaving findTriangleContaining(..), returning Edge");	
		inside= true;
		return (Object) e; 
		}*/
     ...
     * need to worry about them, because they get deleted automagically by this method.
     *
     * @param t a triangle on the boundary that is possibly no longer Delaunay compliant 
    ...
    /** Insert a interior/ exterior Node and update mesh to remain Delaunay compliant.*/ 
    private void insertNode(Node n, boolean remainDelaunay) {
	Triangle t, t1, t2= null, t3, t4= null, oldt1, oldt2;
	Object o;
	Edge e, e1= null, e2, e3, e4= null, e12, e22= null, e32, e42= null, 
	  eOld, b0= null, b1= null;
