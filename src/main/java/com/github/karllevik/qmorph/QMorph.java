...
// ==== ---- ==== ---- ==== ---- ==== ---- ==== ---- ==== ---- ==== ---- 
/**
 * This is the main class, implementing the triangle to quad conversion
 * process. The algorithm was invented by Steven J. Owen, Matthew L. Staten,
 * Scott A. Cannan, and Sunil Saigal and described in their paper "Advancing 
 * Front Quadrilateral Meshing Using Triangle Transformations" (1998).
 * @see <a href="http://www.andrew.cmu.edu/user/sowen/abstracts/Ow509.html" TARGET="_top">the abstract of the paper</a>
...
public class QMorph extends GeomBasics {
    ...
    /** Count the number of front edges in the new loops created if we were to create a 
     * new quad with edge b as base edge and one or both of edges l and r were promoted 
     * to front edges and one or both of their top nodes were located on the front. 
     * Return a byte signifying which, if any, of these loops contains an odd number of 
     * edges.
     *
     * @param b the base edge of the quad we want to make. Should be a front edge. 
     ...
     * that have an odd number of edges. Also indicate if the two loops are actually the
     * same.
     */
    ...
    /** Supposing that the edges side and otherSide are promoted to front edges. The method 
     * parses a new loop involving the edges side, otherSide and possibly edge b. If 
     * two separate loops result from promoting the edges side and otherSide to front edges, then 
     * return the number of edges in the loop involving edges b and side. Otherwise, return the 
     * total number of edges in the loop involving edges side and otherSide.
     *
     * @param b  an edge that is considered for being promoted to a front edge 
	...
	Node n1= side.commonNode(b), n2= otherSide.commonNode(b), 
	...
	/*	if (count1stLoop>= 300)
	    Msg.error("QMorph.countFrontsInNewLoopAt(..): Suspiciously many fronts in loop!");
	*/
     ...
     * @param front2 another front edge adjacent nK and part of the same loop as front1 
    ...
    */
	...
	if (seqQuads== 2) { // || nK.nrOfFrontEdges()> 2) {
		...
		for (int i= 0; i< nK.edgeList.size(); i++) {
		    e= (Edge)nK.edgeList.get(i);
	...
	else // Only step 1 should execute here:
	    newNode= nK.blackerSmooth(nJ, front1, front2, eD.length());
    ...
    /** Calculate smoothed position of node.
     * (Called from localSmooth)
	...
	if (q.hasFrontEdgeAt(n) && !n.boundaryNode()) {
		...
		newN= null; front1= null; front2= null; e= null;
	    }
    ...
    /** Smooth as explained in Owen's paper
     * Each node in the newly formed quad is smoothed. So is every node directly
     * connected to these.
     */
	...
	Node n, nNew, nOld;
	Edge e, fe1, fe2;
	Node bottomLeft, bottomRight, bottomLeftNew, bottomRightNew, 
	    bottomLeftOld, bottomRightOld;
    ...
    /** "Virus" that removes all triangles and their edges and nodes inside of this quad
     * Assumes that only triangles are present, not quads, inside of q
     */
	...
	if (!e.frontEdge && e.isFrontEdge()) {
	    ...
	    if (e.removeFromFront(frontList) && e.level== lowestLevel)
		curLevelEdgesRemoved++;
	    Msg.debug("...removeFromStateList(..) returns "+
		      e.removeFromStateList());
	    Msg.debug("...And the state of the stateLists is :)");
	    Edge.printStateLists();
	    if (e.leftFrontNeighbor!= null && e.leftFrontNeighbor.isFrontEdge())
    ...
    /**
     * Define new fronts, remove old ones.
     * Set new frontNeighbors.
	...
	// We must avoid that nKp1 becomes a boundaryNode because nKp1 is the node that is to
	// be relocated.
	Edge tmp;
	...
	if (safePos== null) {
	    if (q.anyInvertedElementsWhenCollapsed(nKm1, nKp1, nKm1, 
						   nKp1.adjElements(), 
						   nKm1.adjElements())) {
	...
	if (!nKp1.boundaryNode()) {
	    if (safePos!= null)
		nKp1.moveTo(safePos); // e0.midPoint()
	}
	*/
	...
	if (q.largestAngle()> DEG_179) {// Then we can't use the approach in the Owen et al paper! 
	    ...
	    Edge b= q.oppositeEdge(longer), l= q.neighborEdge(b.leftNode, b), 
	...
	Triangle t1New= new Triangle(eF, eFL, eKMid), 
	    t2New= new Triangle(eMidKm1, eMidTT, eTLKm1), 
	...
	Node nKm1= longer.otherNode(nK), nKp1= shorter.otherNode(nK), 
	    opposite= q1.oppositeNode(nK);

	Edge frontBeyondKp1= shorter.frontNeighborAt(nKp1);
	Edge frontBeyondKm1= longer.frontNeighborAt(nKm1);

	Edge q1Top= q1.oppositeEdge(longer), q1nK= q1.neighborEdge(nK, longer), 
	    eKm1Opp= q1.oppositeEdge(q1nK);
	Edge eT1K= t1.neighborEdge(nK, longer), eT1Km1= t1.neighborEdge(nKm1, longer);
	Node c= q1.centroid(), mid= longer.midPoint();	
	Edge eF= new Edge(nK, c), eFL= new Edge(c, mid), 
	    eCOpp= new Edge(c, opposite),
    ...
    */
    ...
    /** @return not null if a special case was encountered. If so, this means that that 
	the base edge has been destroyed, and that a new one has to be chosen. */
    private Quad handleSpecialCases(Edge e) {
	Msg.debug("Entering handleSpecialCases(..)");
	Quad q= null; 
	Triangle eTri= e.getTriangleElement();
	if (!e.boundaryEdge() && !e.leftFrontNeighbor.boundaryEdge() &&
	    !e.rightFrontNeighbor.boundaryEdge()) {
	    
	    // Check for special cases:
	    int nQLeft= e.leftNode.nrOfAdjQuads();
	    if (needsSeam(e, e.leftFrontNeighbor, e.leftNode, nQLeft) && 
		canSeam(e.leftNode, e, e.leftFrontNeighbor)) {
		...
		if (e.isLargeTransition(e.leftFrontNeighbor))
		    ...
		    q= doSeam(e, e.leftFrontNeighbor, e.leftNode);
		     ...
		     e.isLargeTransition(e.leftFrontNeighbor) && 
		...
		if (e.isLargeTransition(e.rightFrontNeighbor))
		    ...
		    q= doSeam(e, e.rightFrontNeighbor, e.rightNode);
		     ...
		     e.isLargeTransition(e.rightFrontNeighbor) &&
    ...
    // jobber her...
    /** If not already set, this method sets the sideEdges, based on state.
     * Called from makeQuad(Edge)
     */
	...
	if (lState== 1 && rState== 1) { // Then side edges are already set.
	    ...
	    /*
	    if (altLSE.size()==1 && altRSE.size()==1) {
		// Make sure that we use legal edges
		e.leftSide= (Edge)altLSE.get(0);
		e.rightSide= (Edge)altRSE.get(0);
	    }
	    */
	}
	...
	if ((
	    (lSide.otherNode(e.leftNode).frontNode() && !lSide.isFrontEdge()) ||
	    (rSide.otherNode(e.rightNode).frontNode() && !rSide.isFrontEdge()))
	    ) {

	    /*	    
	if (evenInitNrOfFronts && (
	    (lSide.otherNode(e.leftNode).frontNode() && !lSide.frontEdge) ||
	    (rSide.otherNode(e.rightNode).frontNode() && !rSide.frontEdge))
	    ) {
	    */
		...
		Msg.debug("nM= "+lSide.otherNode(e.leftNode).descr()+
			  " lies on an opposing front...");
	    else
		Msg.debug("nM= "+rSide.otherNode(e.rightNode).descr()+
			  " lies on an opposing front...");

	    if (/*lSide.getQuadElement()== null &&*/ !lSide.boundaryEdge() 
	    && lSide.otherNode(e.leftNode).frontNode() && !lSide.isFrontEdge())
		lLoop= true;
	    
	    if (/*rSide.getQuadElement()== null &&*/ !rSide.boundaryEdge()
	    && rSide.otherNode(e.rightNode).frontNode() && !rSide.isFrontEdge()) 
		rLoop= true;
	    ...
	    if ((lrc & 1)== 1)
		sideEdges[0]= 
		    lSide.splitTrianglesAtMyMidPoint(triangleList, edgeList, nodeList,e);
	    if (((lrc & 2)== 2) && ((lrc & 4)== 0))
		sideEdges[1]= 
		    rSide.splitTrianglesAtMyMidPoint(triangleList, edgeList, nodeList,e);

	}
	
				/*
				// There are more than one alt side edge for both left and right
				if (altLSE.size()> 1 && altRSE.size()> 1) {
				e.leftSide= defineSideEdge(e, e.leftNode, PIdiv6, altLSE);
				e.leftSide.swappable= false;
				e.rightSide= defineSideEdge(e, e.rightNode, PIdiv6, altRSE);
				e.leftSide.swappable= true;
				}
				// Only right side has more than one alt side edge
				else if (altLSE.size()== 1 && altRSE.size()> 1) {
				e.leftSide= (Edge)altLSE.get(0);
				e.leftSide.swappable= false;
				e.rightSide= defineSideEdge(e, e.rightNode, PIdiv6, altRSE);
				e.leftSide.swappable= true;
				}
				// Only left side has more than one alt side edge
				else if (altLSE.size()>1 && altRSE.size()== 1) {
				e.rightSide= (Edge)altRSE.get(0);
				e.rightSide.swappable= false;
				e.leftSide= defineSideEdge(e, e.leftNode, PIdiv6, altLSE);
				e.rightSide.swappable= true;
				}
				// Neither left or right has more than one alt side edge
				else {
				e.rightSide= (Edge)altRSE.get(0);
				e.leftSide= (Edge)altLSE.get(0);
				}
     ...
     */
    private Edge defineSideEdge(Edge eF1, Node nK, Edge leftSide, Edge rightSide, 
				ArrayList list) {
	...
	curElement= (Element)eF1.getTriangleElement();
	if (curElement== null)
	    Msg.error("...oisann!");
	curElement2= (Element)eF2.getTriangleElement();
	if (curElement2==null)
	...
	if (selAng < EPSILON) {
	    ...
	    Msg.debug("Leaving defineSideEdge(..): Reusing, EPSILON > "+selAng+" ="+
		      Math.toDegrees(selAng)+" degrees");
	    return selected;
	}
	...
	if (selected.otherNode(nK).frontNode()) {
	    ...
	    Msg.debug("Leaving defineSideEdge(..): nM on front, reusing: "+
		      Math.toDegrees(selAng)+" degrees, returning edge "+
		      selected.descr());
	    return selected;
	    /*}
	      else
	...
	Msg.debug("defineSideEdge(..): theta > pi/6 ("+Math.toDegrees(selAng)+
		  " degrees)");
	...
	if (e0.frontEdge ||neighborTriangle== null || neighborTriangle instanceof Quad){ 
	    Msg.debug("Leaving defineSideEdge(..), returning selected=="
		      +selected.descr());
	    return selected;
	}	
     ...
     * Remove all edges intersected by the new edge. This is accomplished through
     * a swapping procedure. All local and "global" updating is taken care of.
	...
	for (int k= 0; k< V.size()-1; k++) {
	    ...
	    if (!T.contains(eK.element1) &&
		(eK.element1== eKp1.element1 || eK.element1== eKp1.element2)) {
		T.add(eK.element1);
	    }
	...
	while (intersectedEdges.size()> 0) {
	    ...
	    // For a stricly convex quad, the nodes must be located on different sides of eI:
		...
		for (int i= 0; i< intersectedEdges.size(); i++) {
		    eI= (Edge)intersectedEdges.get(i);
		    eI.swappable= true;
		}
