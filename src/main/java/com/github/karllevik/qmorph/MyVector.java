...
/**
  This class holds information for vectors, and has methods for dealing with vector-related issues.
*/
...
public class MyVector extends Constants {
    ...
    /** @return angle relative to the x-axis (which is directed from the origin (0,0) to
	the right, btw) in the range (-180, 180) in radians. */ 
    public double angle() {
    ...
    /** @return angle relative to the x-axis (which is directed from the origin (0,0) to
	the right, btw) in the range (0, 360) in radians. */
	...
	BigDecimal d0x= new BigDecimal(x), d0y= new BigDecimal(y), 
	    d1x= new BigDecimal(v.x), d1y= new BigDecimal(v.y);
    ...
    /** Methods to assist stupid Math.acos(..). Tested ok (simple test)
     * Vectors with equal slopes *ARE* considered to be cw to each other. 
     * @param v another vector
    ...
    // Note: java.awt.geom.Line2D has an intersects method.. but no intersectsAt method
    ...
    /** @return if vectors intersects at one point exactly, return a node for this point
	Else return null. */
     ...
     * (that is, a point that is not an endpoint, ) else return false. */
    ...
    /*
    // Return true if this and d1 intersect in a inner point of each other, 
    // (that is, a point that is not an endpoint, ) 
    // else return false.
    public boolean innerpointIntersects(MyVector d1) {
	Node p0= origin, p1= d1.origin;
	MyVector delta= new MyVector(p0, p1.x- p0.x, p1.y- p0.y); 
	...
	BigDecimal d0x= new BigDecimal(d0.x), d0y= new BigDecimal(d0.y), 
	    d1x= new BigDecimal(d1.x), d1y= new BigDecimal(d1.y);
	BigDecimal d0Xd1= d0x.multiply(d1y).subtract(d1x.multiply(d0y));
	if (d0Xd1.compareTo(zero)== 0) {
	    // Non-intersecting and parallel OR intersect in an interval
	    Msg.debug(this.descr()+" doesn't intersect in inner point of "+d1.descr()+" (1)");
	    return false;	
	}
	else {
	    BigDecimal deltax= new BigDecimal(delta.x), deltay= new BigDecimal(delta.y);
	    BigDecimal deltaXd1= deltax.multiply(d1y).subtract(d1x.multiply(deltay));
	    BigDecimal deltaXd0= deltax.multiply(d0y).subtract(d0x.multiply(deltay));  

	    BigDecimal s= deltaXd1.divide(d0Xd1, BigDecimal.ROUND_UP);
	    BigDecimal t= deltaXd0.divide(d0Xd1, BigDecimal.ROUND_UP);
		...
		|| s.compareTo(zero)<=0 || s.compareTo(one)>=0) {
		Msg.debug(this.descr()+" doesn't innerintersect "+d1.descr()+" (2)");
		return false;	// Intersects not at an Edge point (but possibly an interval)
	    }
	    else 
