...
/**
  This class holds information for nodes, and has methods for the management of issues
  regarding nodes.
*/
...
public class Node extends Constants {
	...
	for (int i= 0; i< edgeList.size(); i++) {
	    ...
	    if ((e.leftNode.x > e.rightNode.x) ||
		(e.leftNode.x==e.rightNode.x && e.leftNode.y < e.rightNode.y)) {
		node= e.leftNode;
...
*/
    ...
    /** Note: *ALL* nodes in a neighboring quad is regarded as neighbors, not only those
     * that are directly connected to this node by edges.
     * @return a ccw sorted list of the neighboring nodes to this, but returns null if
     * this node is part of any triangle.
     */
	...
	for (int i= 0; i< elements.size(); i++) {
		...
		Msg.debug("Node.invertedOrZeroAreaElements(..): invertedOrZeroArea: "
			  +elem.descr());
		return true;
	    }
	}
	...
	/*
	  ...
	  cos(alpha) = v_1 * v_2 = (x3 -x2)(x1 -x2) +(y3 -y2)(y1 -y2)
	  cos(beta)  = v_3 * v_4 = (x1 -x4)(x3 -x4) +(y1 -y4)(y3 -y4)
	...
	while (jstart < pattern[0]) {
		...
		if (pattern[j]== 2 &&
		    (pattern2[2]==2 || pattern2[2]==14 || pattern2[2]==0)) {
		    matches= 1;
		    jstart= j;
		    Msg.debug("... rolling pattern...");
		    Msg.debug("...pattern2[2]: "+pattern2[2]+
			      ", pattern["+j+"]: "+pattern[j]);
		    break;
		}
		else if (pattern[j]== 3 &&
		...
		if (pattern[j]== 2 &&
		    ...
		    Msg.debug("...pattern2["+i+"]: "+pattern2[i]+
			 ...
			 (pattern2[i]==3 || pattern2[i]==14 || pattern2[i]==0)) {
		    matches++;
		    Msg.debug("...pattern2["+i+"]: "+pattern2[i]+
			      ", pattern["+j+"]: "+pattern[j]);
		}
		    ...
		    Msg.debug("...pattern2["+i+"]: "+pattern2[i]+
			 ...
			 (pattern2[i]==5 || pattern2[i]==24 || pattern2[i]==0)) {
		    matches++;
		    Msg.debug("...pattern2["+i+"]: "+pattern2[i]+
			      ", pattern["+j+"]: "+pattern[j]);
		}
		...
		/*
		if (pattern2[i]!= pattern[j] && pattern2[i]!= 0 && pattern[j]!= 0) {
		    matches= 0;
		    break;
		}
		else {
