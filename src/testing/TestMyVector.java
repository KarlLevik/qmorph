...
public class TestMyVector extends GeomBasics {
	...
	Msg.debug("Skal være 0:  "+Math.toDegrees(angle0));	

	double angle1= vb.computeAngle(va);
	Msg.debug("Skal være -90:  "+Math.toDegrees(angle1));	

	MyVector vc= new MyVector(p0, -1, -1);
	double angle2= vb.computeAngle(vc);
	Msg.debug("Skal være 135:  "+Math.toDegrees(angle2));	

	MyVector vd= new MyVector(p0, 1, 1.1);
	double angle3= vd.computeAngle(vc);
	Msg.debug("Skal være nær opptil 180:  "+Math.toDegrees(angle3));	

	double angle4= vc.computeAngle(vd);
	Msg.debug("Skal være nær nedtil -180:  "+Math.toDegrees(angle4));	

	MyVector ve= new MyVector(p0, 1, 1);
	double angle5= ve.computeAngle(vd);
	Msg.debug("Skal være ganske lite, pos.:  "+Math.toDegrees(angle5));	
	...
	Msg.debug("Skal være 1:  "+dot0);	


	double dot1= vb.dot(va);
	Msg.debug("Skal være 0:  "+dot1);	

	double dot2= vb.dot(vc);
	Msg.debug("Skal være negativt:  "+dot2);	

	double dot3= vd.dot(vc);
	Msg.debug("Skal være negativt:  "+dot3);	

	double dot4= vc.dot(vd);
	Msg.debug("Skal være nær negativt:  "+dot4);	

	double dot5= ve.dot(vd);
	Msg.debug("Skal være nær positivt:  "+dot5);	
